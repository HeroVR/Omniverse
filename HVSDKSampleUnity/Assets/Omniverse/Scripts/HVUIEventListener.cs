...
public class HVUIEventListener : MonoBehaviour
{
    ...
    static public HVUIEventListener Get(GameObject go)
    {
        HVUIEventListener listener = go.GetComponent<HVUIEventListener>();
        if (listener == null) listener = go.AddComponent<HVUIEventListener>();
        return listener;
    }
    //static public HVUIEventListener Get(GameObject go, PointerEventData eventData)
    //{
    //    HVUIEventListener listener = go.GetComponent<HVUIEventListener>();
    //    if (listener == null) listener = go.AddComponent<HVUIEventListener>();
    //    return listener;
    //}
    static public HVUIEventListener Get(Transform transform)
    {
        HVUIEventListener listener = transform.GetComponent<HVUIEventListener>();
        if (listener == null) listener = transform.gameObject.AddComponent<HVUIEventListener>();
        return listener;
    }
}