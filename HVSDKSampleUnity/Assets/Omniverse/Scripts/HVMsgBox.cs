...
public enum HVMsgBoxTimeoutAct
{
    Close,
    QuitGame,
}

public class HVMsgBox : MonoBehaviour
{
    ...
    public delegate bool EventDelegate(HVMsgBox hmb, string sEvent, GameObject go);
    ...
    HVMsgBoxTimeoutAct _nTimeoutAct;
	...
	protected virtual void Start ()
    {
        ...
        ++_nCounter;        
        if (_nCounter == 1) {
            HVSDK._ControllerRay.CheckShow();
        }
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            if (btn) {
                HVUIEventListener.Get(btn.gameObject).onClick = this.OnClick;
            }
        }
    ...
    {   
        if (_nTime > 0)
        {
            ...
            if (_nTime <= 0)
            {
				...
				{
					if (_nTimeoutAct == HVMsgBoxTimeoutAct.QuitGame) {
						Application.Quit();
					}
	...
	private void LateUpdate()
	{
		...
		{
            _parent.position = HVSDK._ControllerRay._MainCamera.transform.position;
            _parent.rotation = HVSDK._ControllerRay._MainCamera.transform.rotation;
		}
	}
    ...
    public void Reset(string sUsage = "", string sTitle = "", string sContent = "", string sYes = "", string sNo = "", int nTimeout = 0, HVMsgBoxTimeoutAct nTimeoutAct = HVMsgBoxTimeoutAct.Close)
    {
        if (null == _parent)
        {
            _parent = transform.parent.transform;
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            if (btn)
            {
                if (btn_count != t[i]) {
                    ...
                    HVUIEventListener.Get(btn.gameObject);
